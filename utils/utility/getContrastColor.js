...
function parseColor(input) {
  ...
  } else if (cssColors[input.toLowerCase()]) {
    return parseColor('#' + cssColors[input.toLowerCase()])
  } else {
    return false
  }
}
...
function getContrastColor(input) {
  ...
  if (color) {
    const whiteContrast = getContrast(color, [255, 255, 255])
    const blackContrast = getContrast(color, [0, 0, 0])
